...
# Generate the grid
    ...
    def count_nop(self):
        ...
        cell_offset_gdf = self.cell_gdf.copy()
        cell_offset_gdf["geometry"] = self.cell_gdf.geometry.buffer(-0.1)
        cell_offset_gdf = cell_offset_gdf.explode(index_parts=False).reset_index(drop=True)
        cell_boundaries_gdf = gpd.GeoDataFrame(geometry=cell_offset_gdf.boundary.explode(), crs=cell_offset_gdf.crs)

        # Partition each cell boundary into edge segments, keeping cell ownership.
        segment_records = []
        for _, row in cell_offset_gdf.iterrows():
        ...
        partitioned_gdf = gpd.GeoDataFrame(segment_records, crs=cell_offset_gdf.crs)
        ...
        # Boundary edges of each cell that are crossed by the survey
        points_buffer.reset_index(drop=True, inplace=True)
        boundaries = gpd.sjoin(points_buffer, partitioned_gdf, how='inner', predicate='intersects')
        ...
        boundary_counts = (
            boundaries.groupby("cell_idx")["seg_idx"]
            .nunique()
            .reset_index(name="boundaries")
        )

        # Deduplicate corner hits: one survey point can touch two boundary edges.
        joined_gdf = gpd.sjoin(points, self.cell_gdf, how='inner', predicate='intersects')
        joined_gdf = joined_gdf[joined_gdf.geometry.geom_type == "Point"].copy()
        joined_gdf["point_key"] = joined_gdf.geometry.apply(
            lambda p: (round(p.x, 2), round(p.y, 2))
        )
        intersection_counts = (
            joined_gdf.groupby("cell_idx")["point_key"]
            .nunique()
            .reset_index(name="intersections")
        )
